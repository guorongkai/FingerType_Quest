...
2. 专用指令保持简短而直接：清晰、正式的成年美式女声；单词逐音自然读一次，不添加或重复；句子按原文自然读一次。避免把音色、节奏、语调和禁止项堆成超长提示，防止生成式模型因过度约束而错读词素。正常请求固定使用 seed `1`；仅在安全验证失败后才固定使用恢复 seed `0`。如果已复现某个孤立词的拼写转音素错误，Worker 可以在 `WORD_PRONUNCIATION_HINTS` 中加入仅供上游 TTS 使用的音节分隔写法（当前 `napkin → nap-kin`）；页面展示、词库和答案校验始终保留原词，且每次增加提示都会更新语音配置版本以避开旧缓存。
