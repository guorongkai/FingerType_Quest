...
- 宽桌面上，`.practice-row`、中间练习区和 Stats 共同使用 `clamp(132px, 29dvh, 216px)` 高度：单词与句子模式完全一致，较高视口最多为 216px，浏览器变矮时二者同步线性收缩至最小 132px，既保留对齐也优先给键盘空间。窄屏上下堆叠时不施加该共同高度。
...
- `.stats` 是 size container；标题、标签和计数器使用 Stats 容器的 inline/block query units（`cqi` / `cqb`）适配。数值采用 tabular figures，并在 132px 到 216px 的面板高度范围内随容器一并收小，不会保持过大的固定字号或溢出统计卡片。
...
- Typing Coach 顶栏有三列：`Finger Guide + Keyboard`、居中的错误行为说明、以及 `Use:` 当前手指和按键。键盘下方的错误行为说明已移至此处，不再占用键盘框高度。顶栏的标题与当前提示同为 28px 高；按键徽章默认是 21px 小方格，依据按键文字自动横向增长（最多 152px），字号为 `0.65rem`，以完整显示 `Command` 等较长键名。Home Row 提示与键盘之间仅保留 3px Grid gap 和 2px 上内边距。动作与技巧文本继续在 DOM 中更新，但在所有窗口高度下都不占用可视高度。
- 当任意视口高度不超过 760px 时，`.coach-head` 整行隐藏，包含 `Finger Guide + Keyboard`、中间错误说明、`Use:` 手指文字与黄色按键徽章；`.coach` 同时移除顶栏与键盘框之间的 gap，让完整键盘获得最高优先级。
- 顶部使用与键盘断点无关的默认流式规格：`FingerType Quest`、品牌标记、右侧操作按钮，以及 Bank/Mode/Round/Sound/Voice 的标签和控件均用基于 `vw` 与 `dvh` 的 `clamp()` 连续计算。右侧操作按钮在紧凑基线上扩大 20%；Bank、Mode、Round、Sound、Voice 的标签、控件、文字和开关在紧凑基线上扩大 30%。Sound 与 Voice 的圆形开关 thumb 以轨道的垂直中心定位，开启时仅追加水平位移，因此两种状态和所有响应式尺寸下都与轨道居中对齐。这些尺寸会随浏览器宽高平滑变化，不会因键盘断点突然切换。
- 键盘同样不再依赖高度断点的覆盖值：`--key-height` 固定为 `clamp(20px, 5.1dvh, 42px)`，键帽字号、数字与功能键字号、键帽底边、行距、键盘间距、键盘内边距及各手指高度都使用匹配的连续 `dvh` `clamp()` 值。MacBook Neo 常见高度继续获得完整键盘；窗口继续变矮时，所有这些元素按相同方向逐步收小。双符号键仍在较矮窗口改用单层定位：未按 Shift 时主符号略偏左置中，较小的 Shift 符号固定在右上角；按住 Shift 后两者交换主次位置，Shift 符号移到左偏中的主位置并恢复常规字号，原符号缩小为右上角标签。颜色仍分别为蓝色和灰色，状态清晰且不会出现横向空白、标签跳动或窄键帽里的文字重叠。
- Word Practice 的默认目标字、灰色字母格和空格格在原始基线缩小 10% 后，再缩小 10%（累计约 19%）；听写与句子模式仍由各自的专用样式和 `fitTargetText()` 控制，不受这项视觉收紧影响。
- 同一断点把 `.app` 与句子模式 `.app` 的 `align-content` 固定为 `start`。这防止 `min-height: 100dvh` 将自动 Grid 行均匀拉高并在键盘下方制造空白；键盘外框改为自身内容高度，键帽继续随 `dvh` 线性收缩，额外空间只出现在 Coach 下方的页面背景。
...
- `Voice` 开启时当前词优先由 Qwen TTS 朗读，云端暂时不可用时才使用浏览器英文语音保底，且保底过程不改写练习区的提示文本。当前单词播放三遍；每一遍结束后固定静音 3 秒再开始下一遍，因此停顿不受词长或网络响应影响。输入完成或使用 `Next` 后，已经开始的朗读一定会播完，再等待 1 秒才进入下一词，避免旧词被截断或新词过早开始。
- 正确输入后逐字显示。
...
`Voice` 开启时，Word Practice、Dictation 和 Sentence Practice 都优先通过 `/api/tts` 使用 Qwen Breeze 语音；浏览器 `SpeechSynthesis` 只作为 Qwen 临时不可用时的静默保底，不向练习区显示云端故障或降级提示。
...
- Qwen 的 422（主请求和一次简短恢复请求都生成超长、空或超时的词条音频）、429、网络问题、502、504、配置、鉴权或路由错误（例如缺失 Secret、401、404）都只让当次朗读静默降级为浏览器语音；下一次仍会自动尝试 Qwen。页面绝不因一次失败永久禁用 Qwen，也不会把云端故障文字写进练习提示。
...
- 已在 `1280×900`、`1280×800`、`1280×700`、`1280×641` 与 `1280×640` 验证默认流式顶部与放大后的控件：右侧按钮在 700px 高时为约 30.3px；Bank/Mode/Round/Sound/Voice 控件在 700px 高时约 31px，在 641px 高时约 30.2px，全部完整可见。Sound 关闭与 Voice 开启状态的圆形 thumb 在相同的垂直中心线上。键盘在 `761px → 760px` 的键高为 `38.80px → 38.75px`，在 `641px → 640px` 为 `32.69px → 32.63px`；字号、行距、边框和手指高度也连续变化，不再有断点式缩放。`1280×700` 与 `1280×641` 均可完整显示最后一行键，页面无纵向或横向溢出，控制台无错误或警告。
