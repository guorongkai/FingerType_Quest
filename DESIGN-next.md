# FingerType Quest Next 设计文档

## 1. 版本关系

本文件描述新版实验版：`index-next.html`。
...
新版不会覆盖稳定版。稳定版可以继续直接打开 `index.html` 使用；新版为了读取旁边的 Markdown 词库文件，推荐通过本地静态服务打开 `index-next.html`。
...
- 左侧：所有 bank 列表。
- 右侧：选中 bank 的 Markdown 编辑器和对应的导入区。
...
| 个人文件 | Bank ID | 写入时机 |
| --- | --- | --- |
| `current-words.md` | `custom:current` | 编辑或导入 Current Word List 时 |
| `all-added-words.md` | `custom:all` | Current Word List 自动汇总或直接编辑时 |
| `current-sentences.md` | `sentence:current` | 编辑或导入 Current Sentence List 时 |
| `all-imported-sentences.md` | `sentence:all-imported` | Current Sentence List 自动汇总或直接编辑时 |
...
Bank Library 右侧导入区支持两种用途。

### 10.1 保存课程词表

`Save Current Words` 会把导入区文本解析为单词：
...
`Build Sentences` 会把导入区文本解析为句子：

- 覆盖 `Imported Sentences`
- 自动切到 `Sentence Practice`
- 自动选择 `Imported Sentences`
- 可立即练习
...
- 直接双击 `index-next.html` 时，某些浏览器会阻止读取旁边的 Markdown 文件。
- 推荐用本地静态服务打开新版。
- `index.html` 稳定版不需要本地静态服务。
...
- `saveImportAsCurrentWords()`：导入词表到 Current List。
- `saveImportAsSentences()`：导入文本到 Imported Sentences。
...
- `index.html` 稳定版保留，未被新版覆盖。
- 新版保存为 `index-next.html`。
