...
- 同一紧凑键盘断点会把数字键字号缩小到 `0.52rem` 至 `0.7rem`。双符号键采用单层定位：未按 Shift 时主符号略偏左置中，较小的 Shift 符号固定在右上角；按住 Shift 后两者交换主次位置，Shift 符号移到左偏中的主位置并恢复常规字号，原符号缩小为右上角标签。颜色仍分别为蓝色和灰色，状态清晰且不会出现横向空白、标签跳动或窄键帽里的文字重叠。
