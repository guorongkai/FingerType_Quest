...
The practice screen uses the live viewport height to tighten spacing, key sizes, and target text where possible. In Word Practice, the default target word and its gray typing boxes have been reduced by a further 10% for a compact baseline; sentence targets retain their existing full-width treatment and naturally wrap onto lines before the app considers reducing their font size. The title and top actions, plus the Bank, Mode, Round, Sound, and Voice settings, use fluid `clamp()` sizes tied to browser width and height. The top-right actions were enlarged by 20% from the compact setting, and every settings control, including Sound and Voice, was enlarged by 30%; all continue to resize smoothly rather than switching at a keyboard breakpoint. The Sound and Voice switch thumbs are vertically centered within their responsive tracks in both off and on states. The keyboard uses the same approach: key height, keyboard and row gaps, key borders, letter/number/function-key fonts, keybed spacing, and finger overlays share continuous `dvh`-based ranges. This retains a complete keyboard in typical MacBook Neo browser heights while it progressively reduces only as the browser becomes shorter. At larger desktop heights, the center practice and Stats panels top out at `216px`; below that, they shrink together with the viewport down to `132px`, preserving their aligned edges while prioritizing keyboard space. The Stats title, labels, and numerical values use its actual container dimensions, so they scale down with the panel rather than being clipped or remaining oversized. The Typing Coach is kept directly below them, sizes to its keyboard content, and never exceeds `540px`; its compact top row places the Finger Guide label, error-behavior note, and `Use:` key hint together above the keyboard. The Home Row reminder sits closely above the keyboard so the keys receive more vertical space. When the viewport is 760px high or shorter, the Coach top row is hidden to prioritize keyboard space; unmodified dual-symbol keys show the primary symbol slightly left of center with a smaller Shift symbol in the upper right. Holding Shift promotes the Shift symbol to the primary position and size, while the original symbol becomes the smaller upper-right label. Qwen voice failures silently use the selected browser voice without replacing the in-progress practice hint. The app also stops stretching its automatic layout rows, so the keyboard continues to shrink with the viewport instead of gaining blank space below it. Extra browser height remains as page background below the coach. On a very short desktop window, the page becomes vertically scrollable before any target text, Stats cards, controls, or keyboard rows are clipped.
