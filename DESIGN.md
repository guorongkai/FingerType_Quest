...
- 高度至少为 761px 的宽桌面上，`.practice-row`、中间练习区和 Stats 都使用同一个视口感知上限，防止 Grid track 绕过共同限制：普通模式不超过 `min(320px, 35dvh)`，句子模式不超过 `min(360px, 40dvh)`。窄屏上下堆叠时不施加该共同上限；高度不足 761px 的桌面窗口则改用整页滚动来优先保留完整内容。
...
- 已在 `1920×1400` 桌面视口验证面板高度上限：普通练习区与 Stats 均为 320px，句子模式均为 360px；在 `1280×500` 句子模式再次确认不会强制过小上限，整页可滚动，目标文本没有内部溢出。测试期间浏览器控制台无错误或警告。
