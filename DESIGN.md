...
- Typing Coach 顶栏只显示 `Finger Guide + Keyboard`。右侧当前提示缩为同高的一行，显示 `Use:` 与当前手指、以及当前键。按键徽章默认是 26px 小方格，依据按键文字自动横向增长（最多 152px），字号为 `0.72rem`，以完整显示 `Command` 等较长键名。动作与技巧文本继续在 DOM 中更新，但不再占用可视高度。
...
- 已在 `1920×1800` 桌面视口验证紧凑面板与 Coach：单词、句子和 Stats 均保持约 240px；Coach 保持约 418px，并与练习区维持 9px 间隔，视口继续变高时不再增高。Coach 顶栏左右区域均为 34px 高，右侧显示 `Use:`、当前手指和按键；默认 `-` 徽章为 26×26px，`Command` 自动扩为 80px，提示框本身只占约 197px 宽。在 390px 宽窄屏上两者均无溢出。使用长句确认目标会以 30px 字号自然换行且无内部溢出；长统计值 `100 / 100` 的自适应字号为约 19px，未超出卡片。在 `1280×500` 句子模式再次确认不强制过小上限，整页可滚动，目标文本没有内部溢出。测试期间浏览器控制台无错误或警告。
